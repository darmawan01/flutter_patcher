...
export interface AppDeps {
  signer: Signer;
  store: Store;
  /** When set, gates the dashboard/admin endpoints. Device endpoints stay open. */
  adminToken?: string;
}
...
/** Build the Express app over injected deps (no env reads, no listen) so it can
 *  be unit-tested. `index.ts` wires the real signer/store/token and listens. */
export function createApp(deps: AppDeps): express.Express {
  const { signer, store } = deps;
  ...
  // Watch each channel's active patch failure rate; freeze a bad rollout.
  ...
  function requireAdmin(req: express.Request, res: express.Response, next: express.NextFunction): void {
    ...
    if (token && safeEq(token, adminToken)) return next();
  ...
  app.get('/api/meta', (_req, res) => res.json({ authRequired: !!adminToken }));
